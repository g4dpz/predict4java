...
### Fixed
- Timezone and locale issues in SatPassTime date formatting (fixes test failures in non-US locales)
- Date formatters now explicitly use Locale.US and UTC timezone for consistent behavior across all locales
- Resolves issue where PassPredictorTest would fail when run in Australian timezone and locale

### Changed
- SatPassTime date formatters moved to static initializer block with explicit locale and timezone configuration
